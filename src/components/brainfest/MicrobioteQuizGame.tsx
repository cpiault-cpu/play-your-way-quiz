...
import { 
  microbioteLevels, 
  microbioteUiTexts as uiTexts,
  MicrobioteQuestion
} from "@/data/microbioteQuizData";
...
import introImg from "@/assets/microbiote/intro.jpg";
import level1Img from "@/assets/microbiote/level1.jpg";
import level2Img from "@/assets/microbiote/level2.jpg";
import level3Img from "@/assets/microbiote/level3.jpg";
import level4Img from "@/assets/microbiote/level4.jpg";

interface MicrobioteQuizGameProps {
  level: number;
  language: Language;
  onBack: () => void;
...
const MicrobioteQuizGame = ({ level, language, onBack, onLevelComplete }: MicrobioteQuizGameProps) => {
  const { email, setEmail, saveEmail, hasStoredEmail } = useQuizEmail("microbiote");
  ...
  const quizId = `microbiote-${level}`;

  const levelData = microbioteLevels.find(l => l.level === level);
  ...
  useEffect(() => {
    ...
    if (answerTimeLeft <= 0) { handleValidate(true); return; }
  ...
  const handleValidate = useCallback((timedOut = false) => {
    ...
    if (currentQuestion.type === "single") {
      correct = !timedOut && selectedAnswer === currentQuestion.correctIndex;
  ...
  const getLevelColor = () => {
    const colors = ["#7FB3A3", "#E8A87C", "#D17B7B", "#8B5CF6"];
  ...
  const renderAssociationQuestion = (q: MicrobioteQuestion) => {
      ...
      <div className="space-y-4">
        {pairs.map((pair) => (
          <div key={pair.left} className="flex items-center gap-3">
            <span className="text-lg font-medium">{pair.left}</span>
            ...
            <div className="flex gap-2 flex-wrap">
              {rightOptions.map((opt) => {
                ...
                return (
                  <button key={opt} onClick={() => handleAssociationSelect(pair.left, opt)} disabled={showFeedback}
  ...
  const renderMultiQuestion = (q: MicrobioteQuestion) => {
    ...
    return (
      <div className="space-y-3">
        ...
        {options.map((option, index) => {
          ...
          return (
            <motion.button key={index} onClick={() => handleToggleMulti(index)} disabled={showFeedback}
  ...
  const renderSingleQuestion = (q: MicrobioteQuestion) => {
    ...
    return (
      <div className="space-y-3">
        {options.map((option, index) => {
          ...
          return (
            <motion.button key={index} onClick={() => { if (!showFeedback) setSelectedAnswer(index); }} disabled={showFeedback}
        ...
        <AnimatePresence mode="wait">
          ...
          {phase === "email" && (
            <motion.div key="email" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              ...
              <img src={introImg} alt="Microbiote" className="w-40 h-40 rounded-2xl object-cover mb-6 shadow-lg" />
              <h2 className="text-2xl font-bold text-foreground mb-2">
                {language === "fr" ? "Microbiote : Le Grand Safari Intérieur" : "Microbiota: The Great Inner Safari"}
              </h2>
              <p className="text-sm text-muted-foreground mb-1">
                {language === "fr" ? "Explorez votre jungle microbienne niveau par niveau !" : "Explore your microbial jungle level by level!"}
              </p>
              <p className="text-muted-foreground mb-8 max-w-md text-sm">
                {language === "fr" ? "Entrez votre adresse email pour commencer l'aventure" : "Enter your email to start the adventure"}
              </p>
          ...
          {phase === "intro" && (
            <motion.div key="intro" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}
              className="flex flex-col items-center justify-center min-h-[60vh] text-center">
              <img src={levelImages[level] || introImg} alt={levelData?.title[language]} 
                className="w-full max-w-md rounded-2xl object-cover mb-6 shadow-lg" />
              <h2 className="text-2xl font-bold text-foreground mb-2">{levelData?.title[language]}</h2>
              <p className="text-sm font-medium mb-2" style={{ color: getLevelColor() }}>
                {levelData?.subtitle[language]}
              </p>
              <p className="text-sm text-muted-foreground mb-2">
                {currentVersion === "B" ? (language === "fr" ? "🔄 Niveau correctif" : "🔄 Corrective level") : `${uiTexts.levelProgress[language]} ${level}/4`}
              </p>
          ...
          {phase === "final" && (
            <motion.div key="final" initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }}
              className="flex flex-col items-center justify-center min-h-[60vh] text-center">
              <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 w-full max-w-lg">
                <img src={introImg} alt="Safari terminé" className="w-32 h-32 rounded-2xl object-cover mx-auto mb-6 shadow-lg" />
                <h2 className="text-2xl font-bold text-foreground mb-3">{uiTexts.finalTitle[language]}</h2>
                <p className="text-base text-muted-foreground mb-8 whitespace-pre-line">{uiTexts.finalMessage[language]}</p>
...
export default MicrobioteQuizGame;
