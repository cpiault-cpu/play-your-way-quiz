...
import { ArrowRight, Check, X, ArrowLeft, Clock, ExternalLink } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Language } from "@/data/quizData";
import { 
  medicinalPlantsQuizzes, 
  medicinalPlantsTranslations, 
  MedicinalPlantsQuestion 
} from "@/data/medicinalPlantsQuizData";
import { toast } from "sonner";
import { supabase } from "@/integrations/supabase/client";

const TIMER_DURATION = 30;

interface MedicinalPlantsQuizGameProps {
  levelId: "1.1" | "1.2" | "1.3";
  language: Language;
  onBack: () => void;
}
...
const MedicinalPlantsQuizGame = ({ levelId, language, onBack }: MedicinalPlantsQuizGameProps) => {
  const t = medicinalPlantsTranslations[language];
  const quiz = medicinalPlantsQuizzes.find(q => q.levelId === levelId);
  ...
  if (!quiz) {
    return (
      <div className="min-h-screen forest-bg flex items-center justify-center">
        <p className="text-foreground">Quiz not found</p>
      </div>
    );
  }
          ...
          {gameState === "results" && (
            ...
            >
              ...
              {wrongAnswers.length > 0 && (
                <div className="bg-destructive/10 rounded-lg p-4 sm:p-6 mb-6 text-left">
                    ...
                    {wrongAnswers.map((wa, idx) => (
                      <div key={idx} className="bg-background rounded-md p-3 border border-destructive/20">
                        <p className="text-xs sm:text-sm font-medium text-foreground mb-1">
                          {t.question} {wa.questionIndex + 1}: {wa.question}
                        </p>
              ...
              <div className="bg-primary/5 rounded-lg p-4 sm:p-6 mb-6 text-left">
                <h3 className="text-base sm:text-lg font-semibold text-foreground mb-4 flex items-center gap-2">
                  📚 {t.explanationsTitle}
                </h3>
                <div className="space-y-4">
                  {quiz.questions.map((q, idx) => (
                    <div key={idx} className="bg-background rounded-md p-4 border border-border/30">
                      <p className="text-sm font-medium text-foreground mb-2">
                        {t.question} {idx + 1}: {q.question[language]}
                      </p>
                      ...
                      >
                        ...
                        {t.scientificSource}: {q.source.title}
                      </a>
                    </div>
                  ))}
                </div>
              ...
              <Button
                onClick={onBack}
                className="btn-primary-custom text-white w-full text-sm sm:text-base"
              >
...
export default MedicinalPlantsQuizGame;
