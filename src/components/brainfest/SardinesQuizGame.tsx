...
const playValidationSound = (isCorrect: boolean) => {
  try {
    ...
    if (isCorrect) {
      // Pleasant "plouf" sound for correct answer
      oscillator.frequency.setValueAtTime(523.25, audioContext.currentTime); // C5
      oscillator.frequency.exponentialRampToValueAtTime(392, audioContext.currentTime + 0.1); // G4
...
const SardinesQuizGame = ({ level, language, onBack, onLevelComplete }: SardinesQuizGameProps) => {
  ...
  const getSuccessMessage = () => {
    switch (level) {
      case 1: return successMessages.levelComplete[language];
      case 2: return successMessages.level2Complete[language];
      case 3: return successMessages.level3Complete[language];
      case 4: return successMessages.finalComplete[language];
      default: return successMessages.levelComplete[language];
    }
  };
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        background: `linear-gradient(135deg, ${levelData?.color}40 0%, ${levelData?.color}20 50%, white 100%)`
      }}
    >
      {/* Header */}
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-lg font-bold text-foreground">
              {language === "fr" ? "Petites Sardines" : "Little Sardines"} - {levelData?.title[language]}
            </h1>
          </div>
              ...
              <div className="w-full mb-6">
                <div className="flex items-center justify-between mb-2">
                  ...
                  <span 
                    ...
                    style={{ color: readingTimeLeft <= 5 ? "#D17B7B" : levelData?.color }}
                  >
                    {readingTimeLeft}s
                  </span>
                </div>
                <Progress 
                  ...
                  className="h-3"
                  style={{ 
                    ['--progress-color' as string]: readingTimeLeft <= 5 ? "#D17B7B" : levelData?.color
                  }}
                />
              </div>
          ...
          {phase === "results" && (
            ...
            >
              ...
              <div 
                className="w-24 h-24 rounded-full flex items-center justify-center mb-6"
                style={{ 
                  backgroundColor: isPerfect ? `${levelData?.color}40` : "#E8A87C20"
                }}
              >
                <span className="text-4xl font-bold" style={{ color: isPerfect ? levelData?.color : "#E8A87C" }}>
              ...
              <div className="text-center mb-8">
                {isPerfect ? (
                  ...
                  <p className="text-lg text-muted-foreground">
                    {successMessages.tryAgain[language]}
                  </p>
                )}
              </div>
...
export default SardinesQuizGame;
